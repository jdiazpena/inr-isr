...
| Versioned future-constraint interface | New implementation | No scientific constraint was adapted from the legacy repository | The protocol supplies only an engineering boundary and identity/state contract; it implements no plasma transport, precipitation, or imaging equation | A deliberately non-scientific test constraint proves evaluation, history, and checkpoint wiring without making a scientific claim | Permit later FONDECYT-stage work to be added explicitly without implying that it exists in the present UAI-stage implementation |
| PFISR 4D reader and geometry | Clean implementation after individual inspection | `inr-isr/src/amisr_h5_reader_4d.py`; `inr-isr/src/inr_radar/datasets/amisr_h5.py`; copied 3D `src/amisr_h5_reader_3d.py` | Avoids the ambiguous full-field dataset contract and does not infer cadence from filenames; uncertainty is retained without replacement and filtered only by explicit policy | Minimal-HDF geometry/timing tests, explicit uncertainty-filter test, and bounded reads of both actual products with exact metadata checks | Provide one immutable observation adapter for the canonical trainer while preserving actual record timing |
| Named synthetic 4D fields and integration products | Clean implementation informed by individually inspected alternatives | `inr-isr/src/synthetic_plasma_4d.py`; `inr-isr/src/inr_radar/datasets/synthetic_generator_4d.py`; copied `src/synthetic_plasma.py` | Keeps `gaussian_3d` and `chapman_f2_reference` separate; integration averages linear density by a declared rule and does not substitute midpoint truth | Analytic values/derivatives, determinism, instantaneous target, and exact 120/300-second integration tests | Create controlled truth with explicit morphology, motion, and temporal-smearing semantics |
| Independent synthetic evaluator | New implementation | Metric concepts inspected in copied `src/synthetic_analyze_reconstruction.py`; no standalone evaluator copied | Uses independently sampled truth points rather than fitting or collocation points; reports smoke capability, not scientific success | Independent-point identity tests plus bounded instantaneous/120/300-second smoke artifacts containing log, linear, relative, and analytic-derivative metrics | Separate scientific evaluation from training residuals |
| Manifest runner and output contracts | New implementation | Copied 3D shell/Python benchmark orchestration was inspected for operational requirements; legacy 4D standalone scripts were not copied | The runner never treats pilot/long dry-runs as results and refuses in-place restart/overwrite | Strict schema, deterministic IDs, dependency/skip/failure checks, no-side-effect dry-run test, CLI override test, and eight-case bounded smoke | Provide one reproducible orchestration path for synthetic and real adapters |
