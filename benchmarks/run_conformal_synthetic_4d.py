# -*- coding: utf-8 -*-
"""
benchmarks/run_conformal_synthetic_4d.py

Synthetic 4D Held-Out Radar Beam Conformal Calibration Benchmark.
Ingests 4D synthetic beam dataset, splits beams into train/calib/test sets via split_beams,
trains 4D SIREN ONLY on train_beams, calibrates q_0.95 on calib_beams, and evaluates empirical
coverage and interval width on test_beams.
"""
...
from inr_radar.datasets.synthetic_generator_4d import generate_synthetic_beam_dataset_4d
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Synthetic 4D Held-Out Beam Conformal Calibration")
    parser.add_argument("--config", type=str, default="configs/conformal_synthetic_random_config.json")
...
def run_synthetic_4d_conformal_benchmark(config: dict) -> dict:
    seed = config.get("seed", 42)
    set_seed(seed)

    output_dir = Path(config.get("output_dir", "outputs/conformal_synthetic_random"))
    output_dir.mkdir(parents=True, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # 1. Ingest Synthetic 4D Beam Dataset
    n_beams = config.get("n_beams", 42)
    n_ranges = config.get("n_ranges", 25)
    n_times = config.get("n_times", 10)
    z_min_km = config.get("z_min_km", 100.0)
    z_max_km = config.get("z_max_km", 500.0)
    duration_sec = config.get("duration_sec", 3600.0)

    print(f"Generating synthetic 4D dataset with {n_beams} beams across {n_times} time steps...")
    df_full = generate_synthetic_beam_dataset_4d(
        background_ne_m3=config.get("background_ne_m3", 2e11),
        n_beams=n_beams,
        z_min_km=z_min_km,
        z_max_km=z_max_km,
        n_ranges=n_ranges,
    ...
    cluster_center_xy = config.get("cluster_center_xy", [0.0, 0.0])
    cluster_radius_km = config.get("cluster_radius_km", 150.0)
    ...
    # 3. Setup Coordinate Normalization
    ...
    # 4. Initialize and Train 4D SIREN Model ONLY on Train Beams
    model = MLPINR(
    ...
    history = []
    print(f"Training 4D SIREN model on {len(df_train)} train points for {num_steps} steps...")
    ...
    log_text = (
        f"Synthetic 4D Conformal Calibration Verification Log\n"
        f"===================================================\n"
    ...
    print("\nSynthetic 4D Conformal Benchmark completed successfully!")
    print(log_text)
    return metrics
...
def main() -> None:
    ...
    run_synthetic_4d_conformal_benchmark(config)
