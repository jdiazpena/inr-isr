...
class MovingGaussianPatch:
    ...
    def center(self, t_sec: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """Return the advected patch center at each requested time."""
...
def make_time_array(duration_sec: float, n_times: int) -> np.ndarray:
    """Return uniformly spaced observation-center times including both endpoints."""
...
def make_random_xy(
    ...
    """Generate a reproducible uniform random observation geometry."""
...
def make_observation_geometry(
    ...
    """Dispatch a named geometry mode to its coordinate builder."""
...
def plot_truth_slice(
    ...
    """Plot one dense truth slice with its sparse observations."""
...
def choose_plot_time_indices(n_times: int) -> list[int]:
    """Choose unique first, middle, and last diagnostic times."""
...
def make_default_patches(
    ...
    """Construct the requested ordinary-motion or flow-reversal morphology."""
...
def generate_synthetic_case(args: argparse.Namespace) -> dict[str, Path]:
    """Generate observations, truth slices, geometry, config, and plots."""
...
def build_parser() -> argparse.ArgumentParser:
    """Build the synthetic-field generation command-line interface."""
...
def main() -> None:
    """Generate one synthetic case from command-line arguments."""
